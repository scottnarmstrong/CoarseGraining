...
/-!
# Quenched homogenization above a random minimal scale

This is a Mathlib-only comparator challenge for the uniformly elliptic
quenched comparison theorem.
...
coefficient law which is stationary under integer translations, unit-range
dependent, and invariant under signed coordinate changes and adjoints has:
...
* the random-medium assumptions are presented in one flat structure.
