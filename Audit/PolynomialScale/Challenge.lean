...
/-!
# The polynomial homogenization scale for high-contrast media

This is a Mathlib-only comparator challenge for the unconditional
homogenization-scale capstone.

The mathematical content is the following.  In every dimension `d ≥ 3` there
are dimensional constants `Cscale, Ctriadic, alpha > 0` such that every
`Θ`-elliptic (`Θ ≥ 1`) coefficient law which is stationary under integer
translations, unit-range dependent, and invariant under signed coordinate
changes and adjoints admits an entry scale `N₀ : ℕ` with:

* geometric decay of the scalar contrast above the entry scale,
  `θ_{N₀+n} - 1 ≤ 3 ^ (-alpha * n)`;
* a logarithmic bound on the entry scale, `N₀ ≤ Cscale * log (2 + Θ)`; and
* a polynomial bound on the physical entry scale, `3 ^ N₀ ≤ (2 + Θ) ^ Ctriadic`.

The scalar contrast `θ_n = thetaAtScale P n` is the `(0,0)`-entry ratio of the
two annealed conductivity matrices `annealedSigmaAtScale P n` and
`annealedSigmaStarAtScale P n`.  Those are assembled from the coarse block
matrix, which is in turn the polarization of the coarse-graining variational
quantity `Mu`: an infimum of averaged block energies over the block states
whose potential and flux parts have the prescribed means.  Under the structural
hypotheses both annealed matrices are scalar multiples of the identity, so the
`(0,0)`-entry ratio is the scalar contrast.

Ellipticity enters only through the quadratic-form class `IsEllipticMatrix 1 Θ`:
coercivity `∀ ξ, |ξ|² ≤ ξ ⬝ A ξ` together with the inverse quadratic-form bound
`∀ ξ, Θ⁻¹ |ξ|² ≤ ξ ⬝ A⁻¹ ξ`.  The coefficient fields are general, possibly
non-symmetric, matrix fields; no symmetric-ordering phrasing appears anywhere.

Only definitions needed to read that assertion occur below.  In particular:

* invariance of a law is stated directly as equality in distribution of the
  underlying coefficient fields; no carrier-preservation lemmas are exposed;
* the assumptions on the random medium are presented in one flat structure;
* the cube API is cut down to the geometry the statement actually mentions.
