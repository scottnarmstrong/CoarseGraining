...
This file is a **verbatim copy** of the statement vocabulary of
`Audit/PeriodicGeneral/Challenge.lean` (everything in that file except the
final theorem), reproduced here so that `Solution.lean` can state the audited
theorem with a byte-identical statement without importing the challenge module
(which would clash on the theorem name).

The file is deliberately **self-contained** and Mathlib-only: it imports
`Mathlib` and nothing else, exactly as the challenge does.  This is what makes
the vocabulary elaborate in an environment identical to the challenge's, so
that every constant of the audited statement's dependency closure — in
particular the `Module ℝ (Fin d → ℝ)` instance appearing inside `fderiv` in
the `weakFDeriv` field of `WeakH1`/`WeakH10` — has the same instance path in
both environments.  Importing the repository here instead makes those
instances elaborate through repository-provided paths, which the comparator
reports as `Const does not match between challenge and target`.

The repository theorem module and the `attribute [-instance]` erasures of the
ambient repository `MeasurableSpace` instances live in `Solution.lean`, which
imports this file and adds the private bridges.
