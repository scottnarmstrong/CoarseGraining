...
theorem periodicGeneral_comparison
  ...
  have hperRepo :
      _root_.Homogenization.Examples.Periodic.IsPeriodicCoeffField a₀ := by
    intro z
  ...
  have hisoRepo :
      _root_.Homogenization.Examples.Periodic.IsIsotropicCoeffField a₀ := by
    intro R hR
  ...
  have hadjRepo :
      _root_.Homogenization.Examples.Periodic.IsAdjointInvariantCoeffField a₀ := by
  ...
  have hellRepo :
      ∀ Q : _root_.Homogenization.TriadicCube d,
        _root_.Homogenization.Book.Ch04.AEEllipticOn lam Lam
          (_root_.Homogenization.openCubeSet Q) a₀ := by
    intro Q
  ...
  let Srepo : _root_.Homogenization.Book.MainResults.Setup d :=
    _root_.Homogenization.Examples.Periodic.periodicSetup
      two_le_dim a₀ lam Lam hperRepo hisoRepo hadjRepo hlam hle hellRepo
  let sigmaBar : ℝ :=
  ...
  obtain ⟨_sigmaBar, _hsigma, X, hX, hmainS⟩ :=
    hmain two_le_dim a₀ lam Lam hperRepo hisoRepo hadjRepo hlam hle hellRepo
  refine ⟨sigmaBar, hsigma, X, ?_, ?_⟩
  · simpa [Srepo, IsPeriodicMinimalScale, periodicThetaHat,
