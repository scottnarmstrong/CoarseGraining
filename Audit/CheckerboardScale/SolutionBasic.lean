...
challenge `Audit/CheckerboardScale/Challenge.lean`: the ambient vectors and
matrices, the honest coefficient-field carrier with its observable σ-algebra,
triadic cubes and normalized averages, weak `H¹` and `H¹₀`, the block
formalism with the coarse-graining variational quantity `Mu`, the annealed
coarse matrices and the scalar contrast `thetaAtScale`, and finally the
Bernoulli checkerboard law `RandomCheckerboard.law`.
...
The file is deliberately **self-contained** and Mathlib-only.  It does *not*
import `Audit.PolynomialScale.SolutionBasic`, which declares a different
`Homogenization.StatementAudit.originCube` (integer scale index rather than
the natural-number index this challenge uses); importing it would be a
duplicate declaration and using its `originCube` would silently change the
audited statement.

`Solution.lean` imports this file, adds the private bridges to the repository
theorem, and states the audited theorem.
-/
