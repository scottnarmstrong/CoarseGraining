...
carrier analog of the raw-`CoeffField` `LocalSigma U`: it records exactly the
information about a field carried by linear entry tests localized to `U`.  Unlike
the raw fine `LocalSigma`, it is coarser than the canonical carrier σ-algebra
...
σ-algebra).  Packet P3 retargets `CoeffLaw` to this type. -/
