...
Even reflection, extension by zero, global stopping, local harmonic comparison,
...
/-- The centered-Neumann one-level good-`lambda` bound obtained from the
even-reflected parent problem. -/
theorem sqWeightedMeasure_neumannReflected_oneLevel_tail_originCube
    ...
    (hlevel : neumannReflectedGoodLambdaCutoff
      m depth eps sigma0 u H < level) :
  ...
  obtain ⟨uP, huP_grad, hweakP⟩ :=
    exists_h1Function_neumannEvenReflectionParent_divergence_rhs_originCube
      u hH hweak
  let HP : Vec d → Vec d :=
    cubeCoordinateFoldReflectedVectorField (originCube d m) H
  ...
  have hcutoff :
      Real.sqrt (((2 * (cubeRadius (originCube d m) /
        ...
        ((∫ y, ‖F y‖ ^ (2 : ℕ) ∂volume) +
          (eps⁻¹) ^ (2 : ℕ) *
            ∫ y, ‖gext y‖ ^ (2 : ℕ) ∂volume)) < level := by
    have hglobal := neumannReflectedGoodLambdaCutoff_eq_globalEnergy
  ...
  have hFQ : F =ᵐ[volume.restrict Q] fu := by
    ...
    change HilbertVec.ofVec (uP.grad x) =
  ...
  have hvitali : sqWeightedMeasure F volume (T ∩ D) ≤
    ...
    · intro x hx
      ...
      obtain ⟨_hF, _hHext, hlocalF, hlocalweak⟩ :=
        reflectedParent_oneStoppingBall_inputs (depth := depth)
          (sigma0 := sigma0) (by exact hx.1.2) hr hcutoffx uP HP hHP hweakP'
      have hball := sqWeightedMeasure_oneStoppingBall_le G hq x hr hsigma0
        heps heps_one (lt_of_lt_of_le zero_lt_one hM) hlevel_pos hcutoffx F
        Hext hF hHext
      ...
      have hmono :
          sqWeightedMeasure F volume
            ...
            sqWeightedMeasure F volume
              ({y | M * level < ‖F y‖} ∩
                Metric.closedBall x (5 * radius x)) := by
        apply measure_mono
        intro y hy
        exact ⟨hy.1.1.1, hy.2⟩
      calc
      ...
      exact closedBall_subset_openCubeSet_originCube_succ_of_mem hx.1.2 hr.le
        (by
          have hdenom : 2 ≤ 10 * (3 : ℝ) ^ depth := by
            have hpow : 1 ≤ (3 : ℝ) ^ depth :=
              one_le_pow₀ (by norm_num)
            nlinarith
          exact hcutoffx.trans (div_le_div_of_nonneg_left
            (cubeRadius_pos _).le (by norm_num) hdenom)) hyx
  ...
  have hF_tail (a : ℝ) :
      sqWeightedMeasure F volume ({x | a < ‖F x‖} ∩ P) =
        sqWeightedMeasure fu volume ({x | a < ‖fu x‖} ∩ Q) *
          ((3 : ℝ≥0∞) ^ d) := by
  ...
  have hHP_scalar :
    ...
    congr 1
    funext i
    simp only [HP, cubeCoordinateFoldReflectedVectorField, Pi.smul_apply,
      smul_eq_mul]
    ring
  have hgext_indicator :
      gext = P.indicator (sigma0⁻¹ • hilbertifyVecField HP) := by
    change sigma0⁻¹ • hilbertifyVecField Hext =
      P.indicator (sigma0⁻¹ • hilbertifyVecField HP)
    rw [show hilbertifyVecField Hext =
        P.indicator (hilbertifyVecField HP) by
      simpa only [Hext] using
        hilbertifyVecField_reflectedParentDatumExtension m HP]
    funext x
    ...
    · rw [Set.indicator_of_notMem hx, Set.indicator_of_notMem hx]
      exact smul_zero _
  have hH_source_meas :
      AEStronglyMeasurable (sigma0⁻¹ • hilbertifyVecField H)
        (volume.restrict Q) :=
    (memHilbertVectorL2_hilbertifyVecField hH).const_smul sigma0⁻¹
      |>.aestronglyMeasurable
  have hgext_tail (a : ℝ) :
      sqWeightedMeasure gext volume ({x | a < ‖gext x‖} ∩ P) =
        sqWeightedMeasure g volume ({x | a < ‖g x‖} ∩ Q) *
          ((3 : ℝ≥0∞) ^ d) := by
    have hindicator := sqWeightedMeasure_indicator_tail_inter_eq_of_subset
