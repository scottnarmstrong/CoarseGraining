...
This module assembles source-local color-class concentration bounds into
...
/-- Averaging source-local descendants at scale `k` preserves `Gamma_sigma`
concentration under source unit-range dependence. -/
theorem isBigO_gammaSigma_descendantAverage_of_sourceUnitRangeDependentLaw
  ...
  have hClassCount : ∀ c ∈ colors, 0 < classCount c := by
    ...
    have hnonempty : (descendantsAtScaleScaleColorClass Q k c).Nonempty :=
      scaleColorClass_nonempty_of_mem_image (Q := Q) (k := k) (c := c)
        (by simpa [colors] using hc)
    dsimp [classCount]
  ...
  have hY :
    ...
    intro c hc
    have hcolor :=
      isBigO_gammaSigma_finsetSum_descendantsAtScaleScaleColorClass_of_sourceUnitRangeDependentLaw
...
/-- Averaging source-local descendants at scale `k` preserves `Psi_sigma`
concentration under source unit-range dependence. -/
theorem isBigO_psiSigma_descendantAverage_of_sourceUnitRangeDependentLaw
    {d : ℕ} {Q : TriadicCube d} {k : ℤ}
    {P : SourceCoeffLaw d} [IsProbabilityMeasure P] {σ K : ℝ}
    (hk : k ≤ Q.scale)
    (hP : SourceUnitRangeDependentLaw P)
  ...
  have hY :
    ...
    intro c hc
    have hcolor :=
      isBigO_psiSigma_finsetSum_descendantsAtScaleScaleColorClass_of_sourceUnitRangeDependentLaw
