...
private theorem isLocalRandomVariable_fullBlockMat_of_entries
  ...
  change @Measurable (CoeffField d) (FullBlockMat d) (Homogenization.LocalSigma U) _ X
  rw [@measurable_pi_iff (CoeffField d) (BlockCoord d)
    (fun _ => BlockCoord d → ℝ) (Homogenization.LocalSigma U) (fun _ => inferInstance) X]
  intro α
  rw [@measurable_pi_iff (CoeffField d) (BlockCoord d)
    (fun _ => ℝ) (Homogenization.LocalSigma U) (fun _ => inferInstance) (fun a => X a α)]
  intro β
  exact hX α β
