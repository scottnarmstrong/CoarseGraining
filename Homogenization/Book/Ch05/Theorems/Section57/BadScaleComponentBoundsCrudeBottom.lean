...
/-!
# The crude-bottom bad-scale component

This file proves the concrete crude-bottom row estimate and feeds it into the
weighted constant-row summation lemma.  The finite maxima remain as explicit
cardinality prefactors.
-/

noncomputable section

/-- Concrete crude-bottom row estimate. -/
theorem measureReal_shiftedCrudeBottomPairEvent_quenchedProbeEnvelope_le_weighted_row
    {d : ℕ} [NeZero d] {σ : ℝ}
    (hσ_pos : 0 < σ)
  ...
  intro Centry a t αbad P hP hStruct hΓ hσ_eq hparams q r j
  dsimp only
    ...
    · have hℓ_eq : ℓ = selectedBadPairScale K a t αbad q m n := by
      ...
      exact
        measureReal_crudeBottomPairEvent_le_weighted_row_of_badPair_bound
          ...
          hS_nonneg hw_pos.le hD hAρ_nonneg hlam_lower hσ_pos hbad
    · have hempty :
...
/-- Concrete crude-bottom component estimate obtained by summing the weighted
row estimate. -/
theorem measureReal_shiftedCrudeBottomBadScaleEvent_quenchedProbeEnvelope_le_weighted_kernel
    {d : ℕ} [NeZero d] {σ : ℝ}
    (hσ_pos : 0 < σ)
    (params : QuantitativeCoarseGrainedEllipticityParams d) :
    ∃ Ccrude : ℝ, 0 < Ccrude ∧
  ...
  exact
    measureReal_crudeBottomBadScaleEvent_le_weighted_exp_constRows_kernel_of_reindexed_bound
      ...
      (by
        intro r j
        simpa [K, N0, Hshift, S, L, w, A, ρ] using
