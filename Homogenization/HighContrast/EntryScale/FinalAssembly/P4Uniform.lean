...
import Homogenization.HighContrast.EntryScale.EntryScale.P2Uniform

/-!
# Split-exposing, free-entry-slope uniform variant of the final assembly

`exists_final_scale_decay_of_main_buffer_and_rawEnergy_scalars_uniform` is a
**verbatim copy** of the landed
`exists_final_scale_decay_of_main_buffer_and_rawEnergy_scalars`
(`FinalAssembly/P4.lean`) with a ~15-line delta:

* the entry slope is a **free per-law parameter** `q` (with `0 ≤ q`), decoupled
  from the record's `p_hm` (which the landed proof only ever feeds to the
  entry-scale constant); and
* the final scale constant is exposed in the affine form `q + Cdim`, with `Cdim`
  free of `q`, via the `…_split` siblings of the entry-scale chain
  (`EntryScale/P2Uniform.lean`, whose entry slope is quantified inside `∀P`).

Decoupling the entry slope lets the homogenization-scale **uniform** capstone
call this theorem **once** with a `Θ`-free dimensional record (so `Cdim`, `B`,
`C_A`, `λ`, `alpha` are all `Θ`-free, obtained before `∀Θ`), then supply, per
law, a `Θ,T`-dependent entry slope `q := pHmDim` and the shifted-anchor moment
estimate.  The ~450-line constant setup and Lyapunov one-step body are identical
to the landed proof and must be kept in lock-step with it.  The landed theorem
and its 58-tuple consumer in `VarianceUpgrade/P2.lean` are left untouched.
-/
...
/--
Split-exposing, free-entry-slope uniform variant of
`exists_final_scale_decay_of_main_buffer_and_rawEnergy_scalars`.  The entry
slope `q` is a per-law parameter and the final scale constant is `q + Cdim` with
`Cdim` free of `q`.  See the module docstring; the proof is the landed
assembly with the entry-scale chain routed through the `…_split` siblings.
-/
theorem exists_final_scale_decay_of_main_buffer_and_rawEnergy_scalars_uniform
    ...
    ∃ delta_sc C_A lambda B alpha Cdim : ℝ,
  ...
  obtain ⟨Crest, hCrest_nonneg, hentry⟩ :=
    exists_entryScaleConstant_theta_entry_and_memoryGridScale_le_logb_of_choices_initial_split
      (hc := hc) (d := d) (B := B)
      (C_A := C_A) (delta_sc := delta_sc) (lambda := lambda) (L := L)
      hB_one hC_A_pos hdelta_sc_pos hlambda_pos hlambda_lt_one
  obtain ⟨Cdim, alpha, hCdim_nonneg, halpha_pos, hfinal_entry⟩ :=
    exists_uniform_final_scale_decay_and_physical_scale_of_entry_split
      hc loc (delta_sc := delta_sc) (Crest := Crest)
      hdelta_sc_le hCrest_nonneg
  refine
    ⟨delta_sc, C_A, lambda, B, alpha, Cdim,
      hdelta_sc_pos, hdelta_sc_le, hC_A_pos, hlambda_pos, hlambda_lt_one,
      hB_one, halpha_pos, hCdim_nonneg, ?_⟩
  intro P hP hStruct hP4 q N Nstar I _hparams hq_nonneg hN_eq hNstar_eq hI_eq hNNstar
    hHM
  ...
  obtain ⟨hentry_theta, hentry_log⟩ :=
    hentry hP hStruct hP4 hq_nonneg hN_eq hNstar_eq hI_eq hNNstar hA_nonneg hC_A
      (by
        intro i hi hdelta
        simpa [F, H] using hstep_if_above i hi hdelta)
  exact hfinal_entry hP hStruct hP4 hP4_hc hq_nonneg hentry_theta hentry_log
