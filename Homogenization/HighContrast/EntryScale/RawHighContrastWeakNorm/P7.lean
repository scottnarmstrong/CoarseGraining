...
/--
Below-threshold variant of the Section 5.2 low-tail child-response estimate:
the tail threshold `N0` is decoupled from the child-response window `(k, m)`.
Restricting the tail to scales `n < N0` (in the final assembly, `N0 := N`, the
`a.HM` start scale) makes the crude scale-zero gap coefficients carry the
geometric weight `3^{-s'(m-n)} ≤ 3^{-s'(m-N0)}`, which the `Nstar` buffer
kills; the response side stays at the current window.
-/
theorem integrable_section52LowTailBelow_childResponseAverage_special_and_integral_le_responseMoment
    ...
    (N0 : ℕ) {k m : ℕ} (hkm : k < m) (e : Homogenization.Vec d) :
    ...
    let lowSum := fun a : Homogenization.CoeffField d =>
      S.attach.sum fun n =>
        if N0 ≤ Int.toNat n.1 then 0 else lowerSlot a n + upperSlot a n
    ...
    let lowCoeff : ℝ :=
      S.attach.sum fun n =>
        if N0 ≤ Int.toNat n.1 then 0 else
  ...
  have hChildMem :
    ...
    simpa only [childAvg, Q, p_e, q_e, Int.toNat_sub', Int.toNat_natCast] using
  ...
  have hEnvelopeTermInt :
    ...
    intro n _hn
    by_cases hnlow : N0 ≤ Int.toNat n.1
    · simp only [hnlow, ↓reduceIte, enorm_zero, ne_eq, ENNReal.zero_ne_top, not_false_eq_true,
        MeasureTheory.integrable_const_enorm, envelopeTerm]
    · have hsumNested :
          MeasureTheory.Integrable
  ...
  have hLowAE : AEMeasurable lowSum P := by
    have hterm :
        ∀ n ∈ S.attach,
          AEMeasurable
            (fun a : Homogenization.CoeffField d =>
              if N0 ≤ Int.toNat n.1 then 0 else lowerSlot a n + upperSlot a n) P := by
      intro n _hn
      by_cases hnlow : N0 ≤ Int.toNat n.1
  ...
  have hEnvelopeTermBound :
      ∀ n ∈ S.attach,
        ∫ a, envelopeTerm n a ∂P ≤
          coeffResponse *
            ((if N0 ≤ Int.toNat n.1 then 0 else coeffTerm n) * responseMoment) := by
    intro n _hn
    by_cases hnlow : N0 ≤ Int.toNat n.1
    · simp only [hnlow, ↓reduceIte, MeasureTheory.integral_zero, zero_mul, mul_zero, le_refl,
        envelopeTerm, coeffTerm]
  ...
  have hEnvelopeBound :
    ...
    calc
      ...
      _ ≤
          S.attach.sum fun n =>
            coeffResponse *
              ((if N0 ≤ Int.toNat n.1 then 0 else coeffTerm n) * responseMoment) :=
      ...
      _ =
          ...
          calc
            ...
            _ =
              (S.attach.sum fun n =>
                coeffResponse * (if N0 ≤ Int.toNat n.1 then 0 else coeffTerm n)) *
                  responseMoment := by
                rw [Finset.sum_mul]
            _ =
              (coeffResponse *
